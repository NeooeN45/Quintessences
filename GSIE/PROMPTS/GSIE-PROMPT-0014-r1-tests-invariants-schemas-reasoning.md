# GSIE-PROMPT-0014 — R1 — Tests d'invariants des schémas Reasoning
...
| Fichiers possédés | `GSIE/API/tests/unit/test_reasoning_schemas.py` |
| Résultat | 36 tests, tous verts |
...
Démontrer que chaque invariant de `reasoning/schemas.py` rejette ce qu'il doit
rejeter, et accepte ce qui est légitime — un invariant trop strict est un bug.

Couverture exigée : contiguïté de la chaîne d'inférence, fermeture des sources
(citée non déclarée, déclarée non utilisée), cohérence du plancher de preuve,
bornes de `niveau_confiance`, prémisses non vides, contexte non vide, absence de
champ de résolution sur `ContradictionDetectee` vérifiée par introspection, et
acceptation d'un `InferenceResult` sans conclusion — l'absence de résultat est
un résultat honnête.
