# GSIE-PROMPT-0004 — Contre-audit n°2, A1 — Procédure de révision des textes `Locked`
...
Vérifier la clôture du constat **C-01 (P0)**.

Rappel : `RFC-0023` §10 organisait la publication de nouvelles éditions de
documents `Locked` sur la base d'une décision, alors que `GSIE-CON-000` exige
une RFC. Aucune RFC ne contenait le texte constitutionnel révisé.

Correction annoncée : création de `RFC-0025` et `RFC-0026`, enveloppes
destinées à porter les textes cibles.

Questions traitées : les enveloppes contiennent-elles le texte ou seulement la
promesse ? Une enveloppe vide satisfait-elle `GSIE-CON-000` ? La séquence §10
place-t-elle la décision après la RFC portant le texte ? Reste-t-il un chemin
permettant de publier un `Locked` sans RFC portant le diff ? Qui a autorité
pour faire passer `RFC-0025` de `Brouillon` à `Proposé` ?
