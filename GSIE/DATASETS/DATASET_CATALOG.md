...
alimentent la base de connaissances GSIE (`GSIE/KNOWLEDGE/`) et les
14 moteurs spécialisés (`GSIE/ENGINES/`). Il opérationnalise deux
...
| **Classification (11 classes)** | **ASPRS (8)** : 1=non classé, 2=sol, 3=vég. basse (0-50cm), 4=vég. moyenne (50cm-1m50), 5=vég. haute (>1m50), 6=bâtiment, 9=eau, 17=tablier de pont. **IGN (3)** : 64=sursol pérenne (éoliennes, lignes électriques, antennes), 66=points virtuels (sous ponts + cours d'eau >5m), 67=divers bâtis (incertain). Voir `GSIE/RESEARCH/LIDAR_HD_SPECIFICATIONS.md` §4. |
| **Attributs points** | 13 standards (X,Y,Z, Intensity, Return Number, Classification, Scan Angle, GPS Time...) + 3 Extra Bytes (DTM_Marker, DSM_Marker, Origin). Format LAZ 1.4 PDRF 6, indexation COPC. |
| **Accès données** | Téléchargement COPC.LAZ (dalles 1km) + flux EPT/VPC (streaming) + WMS-Raster (MNT/MNS/MNH + ombrages) + API Calcul Altimétrique (REST). Voir `GSIE/RESEARCH/LIDAR_HD_SPECIFICATIONS.md` §5. |
| **Qualité géométrique** | REMQ planimétrique ≤ 50 cm (constaté 11,7 cm), REMQ altimétrique ≤ 10 cm (constaté 5,5 cm). Contrôle classement 20-30% chantier, < 1 bâtiment / 1000 mal classé (zone à enjeu). |
...
> pipeline partiellement autonome (voir `GSIE/ARCHITECTURE/GSIE_IGNIS_*`).
...
- `GSIE/ARCHITECTURE/ENGINE_DEVELOPMENT_ORDER.md` — Ordre de développement des moteurs (livrable 204)
- `GSIE/RESEARCH/RESEARCH_METHOD.md` — Méthode de recherche (livrable 301)
- `GSIE/KNOWLEDGE/KNOWLEDGE_METHOD.md` — Méthode de connaissance (livrable 302)
- `GSIE/KNOWLEDGE/FOREST_ONTOLOGY.md` — Ontologie forestière (livrable 303)
