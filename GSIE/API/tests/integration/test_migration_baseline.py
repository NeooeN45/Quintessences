...
_IMAGE_DB = os.environ.get("GSIE_TEST_DB_IMAGE", "api-db:latest")
...
def _check_avec_faux_positifs(config: object) -> None:
    """Apelle command.check en tolerants les faux positifs connus.

    Faux positifs (baseline Locked immuable, DEC-000036) :
    - modify_default sur revision/outbox/inbox : la baseline crée
      DEFAULT 'now()' (string literal) mais le modèle utilise func.now()
      (function) — equivalence semantique, difference syntaxique.
    - add_index idx_place_geom_4326 : GeoAlchemy2 gere les colonnes
      Geometry Computed differemment ; l'index est cree par la migration
      20260727_0005 et declare dans __table_args__ du modele.
    """
    ...
    try:
        ...
        for mot in mots_interdits:
            assert mot not in msg, f"Diff inattendu '{mot}' detecte par command.check: {msg}"
...
class TestBaselineGSIEV62:
    ...
    def test_upgrade_downgrade_upgrade(self, base_vierge: str) -> None:
        ...
        _check_avec_faux_positifs(config)
