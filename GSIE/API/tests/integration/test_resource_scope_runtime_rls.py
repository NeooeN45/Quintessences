"""Preuve d'acceptation du RLS ``resource`` sous le vrai profil runtime.

Ce module complète ``test_isolement_rgpd.py`` : vérifier qu'une policy existe
ou qu'un rôle possède certains GRANT ne prouve pas que l'application est
réellement enfermée par PostgreSQL. Ici, une base migrée est ouverte avec un
compte LOGIN non propriétaire, membre de ``gsie_application`` et explicitement
``NOBYPASSRLS``. Les scénarios reproduisent ensuite la résolution de contexte de
``get_db_resource`` avant d'éprouver directement ``resource_scope_visible``.
...
def _runtime_url(owner_url: str) -> str:
    ...
    return f"postgresql+asyncpg://{_RUNTIME_ROLE}:{_RUNTIME_PASSWORD}@{host_and_db}"
...
def migrated_database() -> Generator[str, None, None]:
    """Base vierge réellement migrée jusqu'à ``head`` avec rôle runtime dédié."""
    ...
    with PostgresContainer(
        ...
        asyncio.run(
            _execute(
                ...
                (
                    f"CREATE ROLE {_RUNTIME_ROLE} LOGIN PASSWORD '{_RUNTIME_PASSWORD}' "
...
def tenant_ids(migrated_database: str) -> dict[str, UUID]:
    """Deux tenants symétriques, chacun avec un workspace et une resource."""
    ids = {
        "user_a": uuid4(),
        "user_b": uuid4(),
        "org_a": uuid4(),
    ...
    async def seed() -> None:
        ...
        try:
            async with engine.begin() as connection:
                for key in ("user_a", "user_b"):
                    await connection.execute(
                        text(
                            ...
                            "(id, status, session_version) VALUES (:id, 'active', 1)"
                        ),
                        {"id": ids[key]},
                    )
                for suffix in ("a", "b"):
                    ...
                    await connection.execute(
                        ...
                        {"org": ids[f"org_{suffix}"], "account": ids[f"user_{suffix}"]},
                    )
                workspaces = (
                ...
                resources = (
                    ("resource_a", "org_a", "workspace_a", "tenant-a-main"),
                    ("resource_a_other", "org_a", "workspace_a_other", "tenant-a-other"),
                    ("resource_b", "org_b", "workspace_b", "tenant-b-main"),
                )
                for resource_key, org_key, workspace_key, gsie_id in resources:
                    await connection.execute(
                        text(
                            "INSERT INTO public.resource "
                            "(id, organisation_id, workspace_id, type, gsie_id, metadata_json) "
                            "VALUES (:id, :org, :workspace, 'entity', :gsie_id, '{}'::jsonb)"
                        ),
                        {
                            "id": ids[resource_key],
                            "org": ids[org_key],
...
async def _runtime_session(owner_url: str) -> AsyncGenerator[AsyncSession, None]:
...
async def _activate_context(
    ...
    request = _request(**{"X-Organisation-Id": str(organisation_id)})
    resolved_org = await _resolve_active_organisation(session, str(user_id), request)
    ...
    resolved_workspace = await _resolve_active_workspace(
        ...
        _request(
            **{
                ...
                **({"X-Workspace-Id": str(workspace_id)} if workspace_id else {}),
            }
        ),
        None,
    )
...
@pytest.mark.asyncio
async def test_runtime_role_is_non_owner_and_cannot_bypass_rls(migrated_database: str) -> None:
            ...
            row = (
                await connection.execute(
                    text(
                        ...
                        "pg_has_role(current_user, 'gsie_application', 'member') AS app_member, "
                        "pg_get_userbyid(c.relowner) = current_user AS owns_resource "
...
@pytest.mark.asyncio
async def test_foreign_organisation_header_is_rejected_before_rls_context(
    migrated_database: str, tenant_ids: dict[str, UUID]
) -> None:
    async for session in _runtime_session(migrated_database):
        await set_rls_context(session, str(tenant_ids["user_a"]), "reader,writer")
        with pytest.raises(HTTPException) as exc_info:
            await _resolve_active_organisation(
                session,
                str(tenant_ids["user_a"]),
                _request(**{"X-Organisation-Id": str(tenant_ids["org_b"])}),
            )
...
async def test_runtime_rls_hides_foreign_tenant_and_other_workspace(
    ...
    async for session in _runtime_session(migrated_database):
        ...
        visible = set(
            (
                await session.execute(text("SELECT id FROM public.resource ORDER BY id"))
            ).scalars()
        )
...
async def test_runtime_cannot_soft_delete_foreign_resource(
    ...
    async for session in _runtime_session(migrated_database):
        ...
        result = await session.execute(
            ...
            {"now": datetime.now(UTC), "id": tenant_ids["resource_b"]},
        )
...
async def test_rls_context_does_not_leak_between_transactions(
    ...
    engine = create_async_engine(_runtime_url(migrated_database), pool_size=1, max_overflow=0)
    factory = async_sessionmaker(engine, class_=AsyncSession, expire_on_commit=False)
    try:
        ...
        # set_config(..., true) doit avoir disparu avec la transaction.
