"""Tests d'intégration — Knowledge Engine (persistance PostgreSQL réelle).

Couvre : ingestion, requête, versionnement, révision (CON-010), filtres.
Utilise la base PostgreSQL/PostGIS réelle (testcontainers, fixture
partagée dans tests/conftest.py) — le Knowledge Engine persiste désormais
en base (resource + assertion + evidence_assessment + revision, RFC-0011),
et ne peut plus être testé en pur mémoire.
"""
...
@pytest.fixture
def engine(db_session: AsyncSession) -> KnowledgeEngine:
    """Engine adossé à une session Postgres réelle et vide (isolation par test)."""
...
async def should_ingest_knowledge_when_valid_request(engine: KnowledgeEngine):
    """L'ingestion d'une requête valide doit retourner un KnowledgeObject."""
    req = _make_ingest_request()
    result = await engine.ingest(req)
...
async def should_raise_error_when_ingesting_duplicate(engine: KnowledgeEngine):
    """L'ingestion d'un UUID déjà présent doit lever une erreur."""
    req = _make_ingest_request()
    await engine.ingest(req)
    with pytest.raises(KnowledgeEngineError, match="existe déjà"):
        await engine.ingest(req)


async def should_raise_error_when_ingesting_refused_knowledge(engine: KnowledgeEngine):
...
async def should_raise_error_when_ingesting_quarantine_knowledge(engine: KnowledgeEngine):
...
async def should_return_empty_result_when_graph_is_empty(engine: KnowledgeEngine):
...
async def should_return_knowledge_when_querying_by_concept(engine: KnowledgeEngine):
    ...
    await engine.ingest(concept_req)
    await engine.ingest(seuil_req)
...
async def should_filter_by_evidence_min_when_provided(engine: KnowledgeEngine):
    ...
    await engine.ingest(req_b)
    await engine.ingest(req_d)
...
async def should_filter_by_mots_cles_when_par_concept_with_filter(engine: KnowledgeEngine):
    ...
    await engine.ingest(req1)
    await engine.ingest(req2)
...
async def should_paginate_results_when_page_size_exceeded(engine: KnowledgeEngine):
...
async def should_create_new_version_when_revising(engine: KnowledgeEngine):
    """La révision doit incrémenter la version et archiver l'ancienne (CON-010)."""
    req = _make_ingest_request()
    original = await engine.ingest(req)
    ...
    revised = await engine.revise(revision)
...
async def should_raise_not_found_when_revising_unknown_knowledge(engine: KnowledgeEngine):
    ...
    with pytest.raises(KnowledgeNotFoundError):
        await engine.revise(revision)


async def should_raise_error_when_revising_without_changes(engine: KnowledgeEngine):
...
async def should_preserve_history_across_multiple_revisions(engine: KnowledgeEngine):
...
async def should_return_graph_version_when_queried(engine: KnowledgeEngine):
...
async def should_return_correct_count_when_multiple_types_ingested(engine: KnowledgeEngine):
...
async def should_retrieve_by_id_when_filtering(engine: KnowledgeEngine):
...
async def should_return_object_count_when_stats_called(engine: KnowledgeEngine):
    """stats doit retourner le nombre d'objets dans le graphe."""
    assert (await engine.stats())["total_objects"] == 0
    await engine.ingest(_make_ingest_request())
    assert (await engine.stats())["total_objects"] == 1


def should_reject_empty_justification_in_revision():
