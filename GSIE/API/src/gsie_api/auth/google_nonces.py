"""Registre de nonces Google courts et à usage unique."""
...
class GoogleNonceStore(Protocol):
    """Contrat du registre anti-rejeu Google."""
...
class RedisGoogleNonceStore:
    ...
    async def create(self) -> str:
        for _ in range(5):
            ...
            if created:
                return nonce
        raise RuntimeError("Impossible de produire un nonce Google unique")
...
@lru_cache
def get_google_nonce_store() -> GoogleNonceStore:
    """Construit le registre configuré pour le processus."""
    settings = get_settings()
    storage_url = settings.google_nonce_storage_url or settings.refresh_token_storage_url
    ttl_seconds = settings.google_nonce_expire_seconds
    if storage_url == "memory://":
        return MemoryGoogleNonceStore(ttl_seconds)
    return RedisGoogleNonceStore(storage_url, ttl_seconds)


async def close_google_nonce_store() -> None:
