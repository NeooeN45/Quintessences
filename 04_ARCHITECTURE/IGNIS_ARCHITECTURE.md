# Architecture Ignis — Système d'aide à la décision pour la surveillance et l'analyse des incendies de forêt
...
| **Documents connexes** | `GSIE_IGNIS_GCS_CINEMA_UNREAL.md` (livrable 211, GCS-Cinéma UE 5.8), `GSIE_IGNIS_DATA_PIPELINE.md`, `GSIE_IGNIS_DRONE_ARCHITECTURE.md`, `22_PROJECT_MEMORY/Ignis.md` (registre d'idées), `22_PROJECT_MEMORY/Ignis/Phase0_comparatif_moteurs_simulation.md` |

---

## 1. Objet et périmètre

Ce document décrit l'architecture du système cible Ignis : un système
...
Ignis est une **application cliente** de l'écosystème GSIE — ce n'est ni
...
banc de simulation actuel (`~/Ignis/` WSL2, Jalon 0–2) en est une
...
┌──────────────────────────┴──────────────────────────────────────┐
│ Ignis — Application cliente dédiée risque incendie           │
│                                                                 │
│  ┌──────────┐  ┌───────────┐  ┌──────────┐  ┌───────────────┐  │
...
> vivant. Le but de Ignis n'est pas de créer un logiciel de
...
> nécessaire. L'intelligence reste dans Ignis ; le rendu n'est qu'une
...
informations calculées par le serveur Ignis via l'API temps réel
...
> serveur Ignis n'est pas un simple backend : c'est un système
...
> et propose », jamais « décide à la place de l'humain ». Ignis est un
...
Toute nouvelle simulation peut être évaluée. Ignis est une plateforme
...
**Intégration** : PX4 gère le vol de bas niveau. Ignis ne réinvente
...
**Rôle** : interface haut niveau entre Ignis et PX4 — définition de
...
particulaire. C'est la brique différenciante de Ignis.
...
Ignis est une application cliente : elle **sollicite** les moteurs GSIE
...
| Moteur GSIE | Rôle dans Ignis | Modalité | Idées liées |
|---|---|---|---|
...
| **Knowledge Engine** | Intégration des connaissances qualifiées (combustible, topographie, météo, enjeux) dans le graphe de connaissances. Base de doctrine DFCI, RETEX historiques. | Consommation + alimentation : Ignis nourrit le graphe avec les observations terrain et la « boîte noire » du front (J-10). | J-10, D-04, M-21 |
...
> Ignis est un **outil d'aide à la décision** du COS / CODIS, jamais
...
Les jalons Ignis s'inscrivent dans les phases GSIE globales (voir
`22_PROJECT_MEMORY/Ignis.md` §10) :
...
- `22_PROJECT_MEMORY/Ignis.md` — Registre d'idées (60+ idées, 9
  sections)
- `22_PROJECT_MEMORY/Ignis/Phase0_comparatif_moteurs_simulation.md`
  — Comparatif sourcé Phase 0
- `22_PROJECT_MEMORY/Ignis/guide_installation_banc.md` — Guide
  d'installation
- `22_PROJECT_MEMORY/Ignis/journal.md` — Journal de bord
...
> **Rappel final** : Ignis est un outil d'aide à la décision. Le COS
