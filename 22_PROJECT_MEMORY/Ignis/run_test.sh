#!/bin/bash
# Ignis — Lanceur générique de test de vol
# Usage: bash run_test.sh <script_python>
export HOME=/home/camil

SCRIPT_NAME="${1:-premier_vol.py}"
SCRIPT_PATH="/mnt/a/GSIE/22_PROJECT_MEMORY/Ignis/${SCRIPT_NAME}"

if [ ! -f "$SCRIPT_PATH" ]; then
    echo "✗ Script introuvable: $SCRIPT_PATH"
    echo "Usage: bash run_test.sh <script_python>"
    ls /mnt/a/GSIE/22_PROJECT_MEMORY/Ignis/vol_*.py 2>/dev/null | xargs -n1 basename
    exit 1
fi
