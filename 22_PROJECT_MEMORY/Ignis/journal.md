...
- Structure `~/Ignis/` créée (forefire/, PX4-Autopilot/, data/, scripts/,
...
- `propagation.png` généré (première image du projet Ignis)
...
- Script `premier_vol.py` (MAVSDK) : armement → décollage → télémétrie →
  atterrissage
- git init sur ~/Ignis/ (scripts/, assimilation/, gcs-lite/, notes/)
