...
| **Dernière mise à jour** | 2026-08-07 — **Pentest authentification/connexion — clos** (`PENTEST_AUTH_CONNEXION_2026-08-07.md`). Revue statique complémentaire au pentest infrastructure du même jour, centrée sur le code auth/identité/RBAC. 3 constats Moyens tous corrigés : (1) IP incohérente derrière le tunnel Cloudflare — audit trail et lockout utilisaient l'IP interne du tunnel au lieu de l'IP réelle, corrigé via `core.limiter.get_client_address()` dans `identity_router.py`, `router.py`, `audit/middleware.py` ; (2) verrouillage de compte contournable par rotation d'IP — `lockout.py` verrouille désormais aussi par compte seul ; (3) nonce manquant sur le flux OIDC générique — nouveau module `auth/oidc_nonces.py` (miroir de `google_nonces.py`), nonce serveur exigé et vérifié sur `/oidc/{provider}/authorize` et `/login/oidc`. Aucun constat ouvert. 192 puis 117 tests passants, aucune régression. Voir aussi **Veille BEAM/OTP et vérification formelle** (`GSIE/RESEARCH/VEILLE_BEAM_OTP_SERVER_MESHING_2026-08-07.md`, Draft). Synthèse d'une discussion externe sur Erlang/OTP/Elixir/Gleam ; ne change pas le verdict DEC-000019. Deux apports ajoutés au plan de surveillance (patron OTP « supervision par isolation de panne » pour le futur Server Meshing ; **P**/**Dafny** pour la vérification formelle du transfert d'autorité). Voir aussi **Pentest défensif post-déploiement** (`SECURITY_AUDIT_2026-08-07.md`) : Cloudflare/DNS, API, landing, admin, Docker et secrets audités. 2 findings Élevés corrigés en live : comparaison dev login en `hmac.compare_digest` et refus des clés JWT auto-générées en staging/production. Dev login désactivé par défaut dans `.env.example`. Score global 8.2/10. Recommandations restantes **fermées** : enregistrements CAA ajoutés, HSTS preload actif (`max-age=63072000; includeSubDomains; preload`, soumis à hstspreload.org, statut `pending`), Worker rate-limiter `gsie-rate-limiter` déployé sur `api.quintessences-platform.com/*`, `/metrics` protégé par token Bearer optionnel `GSIE_METRICS_BEARER_TOKEN` (sinon admin), tests unitaires `test_metrics.py` ajoutés. Prompt `PROMPT_PENTEST_CLAUDE.md` créé pour audit défensif ciblé. La Global API Key utilisée doit être révoquée. |
