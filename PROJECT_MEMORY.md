...
| **Dernière mise à jour** | 2026-08-02 — **Veille technologique exhaustive + correction post-revue** : 8 sous-agents en parallèle (DB, Moteurs AI/ML, API FastAPI, Géospatial, Observabilité/Sécurité, Concurrence, Infrastructure, Data pipelines). Document de synthèse : `21_EXPERIMENTS/VEILLE_TECHNO_2026-08-02.md` (niveau de preuve D per RFC-0014, statut Draft). **5 écarts avec le dépôt corrigés** après revue : security headers déjà implémentés (`middleware.py:25-33`), pg_stat_statements déjà activé (`docker-compose.yml:36`), Apache AGE déjà déployé, API versioning `/api/v1/` en place, PgBouncer config présent mais non déployé. Chiffres non sourcés marqués comme impressions d'agent. Azure Key Vault reframé comme transition depuis Fernet (97e269d). **Implémentations effectives par le Fondateur** : Trivy + Bandit dans CI (`ci.yml`), Tenacity dans `pyproject.toml`. **Actions Phase 1 réellement à faire** : orjson, uvloop, Dependabot, Key Vault, Uptime Kuma, PlantNet API (7 actions, 3 déjà faites). |
