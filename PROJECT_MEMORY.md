...
| **Dernière mise à jour** | 2026-08-01 — **Audit Phase 4 : correction P0/P1/P2** : (1) **14 tests en échec → 0** : activation `pytest-xdist -n 2 --dist=loadfile` par défaut (isolation par processus worker), fixture autouse `_ensure_fresh_event_loop` (fix Windows `RuntimeError: Event loop is closed`), fixture autouse `_reset_rate_limiter` (fix `429 Too Many Requests` fallacieux), fixture `mock_lifespan` (mock connexions DB/Redis/WebSocket du lifespan). Résultat : `1640 passed, 114 skipped, 0 failed`. (2) **7 erreurs tsc ADMIN_WEB → 0** : ajout `planned_week` + `language` au type `EngineStatusResponse`, 3 `class` → `className` sur SVG React. `tsc --noEmit` exit 0, `npm run build` 12 pages 0 erreur. (3) **P0-1 sécurité** : clé API Météo-France en clair sur disque — rotation requise par l'utilisateur sur portail-api.meteofrance.fr + migration vers secret manager. (4) **Documentation** : `docs/TESTING_XDIST.md` mis à jour. Tête Alembic `20260801_0027` (27 révisions). |
