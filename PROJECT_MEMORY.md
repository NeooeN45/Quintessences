...
| **Dernière mise à jour** | 2026-08-02 — **Correction P1/P2 audit moteurs GSIE** : (1) **P1a tests d'intégration** : 5 nouveaux fichiers (Evidence, Diagnostic, Validation, Climate, Learning) — 47 tests `requires_docker`, pattern `db_session` sans `@pytest.mark.asyncio`. (2) **P1b cache SynopClient** : cache par instance TTL 1h, `_CachedFile` avec `__slots__`, plus de 18 Mo téléchargés à chaque appel. (3) **P1c migration `validation_result` (critique)** : le modèle `ValidationResultModel` existait sans migration — création de `20260801_0028_validation_result_table.py` (table + FK CASCADE + 3 index + COMMENT ON). Tête Alembic `20260801_0028` (28 révisions, 120 tables). (4) **P2a refactor Knowledge** : extraction sous-responsabilités. (5) **P2b persistance Validation Engine** : `ValidationResultModel` + `AsyncSession` passée par le router, persiste `bloque`/`partiellement_valide` pour le Learning Engine (RFC-0028). (6) **P2c commentaire corrigé** : `gsie_id` uuid4 = traçable non reproductible. Validation : ruff OK, mypy OK, 1444 tests passés (8 échecs préexistants Redis, 0 régression). |
